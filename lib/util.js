...
TODO:
  - make an "onenter" handler function, and an agumentation helper to add it
    to an input element
  - make a "form" version of the "view" widget that makes it easy to populate
    a form that is then submitted back to a handler (which could then check,
    format, and post the form contents to a remote resource)
