...
TODO:
  ...
  - give into the dark side, and just start extending the DOM types, already
...
hz.ui = ( function( ns ) {
    ...
    ns.add = {
        ...
        /**
         *  Creates and adds an enhanced file input widget.  The document
         *  structure is intended to allow sweeping changes to the style of
         *  the input via CSS.  The file or files selected are displayed in
         *  nice table within the widget (with types and sizes of each).
         *
         *  @param elem     The element that will contain the input
         *  @param handler  Optional callback handler for the change event
        ...
        'efile' : function( elem ) {
            ...
            var multiple     = arguments.length > 2 ? arguments[ 2 ] : false;
        ...
        /**
         *  Creates and adds a standard file input.
         *
         *  @param elem     The element that will contain the input
    ...
    ns.show_form = function( title, spec, callback ) {
        ...
        for( var key in spec ) {
            spc = spec[ key ];
            if( ( 'type' in spc ) && ( spc[ 'type' ] == 'hidden' ) ) {
                frm.set_hidden( key, spc[ 'value' ] );
            }
            else {
        ...
        hz.util.append( document.body, node );
        first.focus();
        return node;
    };
    ...
    /**
     *  Sets a hidden piece of information in the form return data.
     *
     *  @param key   The key into the form data for retrieval
     *  @param value The value of the item to set
     */
    form.prototype.set_hidden = function( key, value ) {
