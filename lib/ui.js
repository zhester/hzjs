...
hz.ui = ( function( ns ) {
    ...
    /*
     * Helpers for binding interactive actions to document elements.
     */
    ns.bind = {

        /**
         * Binds the enter key to an element that can accept `keydown` events.
         *
         * @param elem     The element to which the handler is bound
         * @param callback The function to call when the key event is
         *                 triggered.  The event object is passed to the
         *                 callback handler.  If the `stop` parameter is not
         *                 set, the callback function may return a non-false
         *                 value to prevent future key event handlers from
         *                 firing.
         * @param stop     Set to always stop future key event handlers from
         *                 firing (default: false)
         */
        'enter' : function( elem, callback ) {
            var stop = arguments.length > 2 ? arguments[ 2 ] : false;
    ...
    ns.create = {
         ...
         *  selection.
         *
         *  ZIH - need to add proxying for additional input properties:
         *          - value, type
         *
         ...
         *  @param attrs   Optional key-value object of proxied attribute
         *                 names and values
        ...
        'check' : function( values ) {
            ...
            var attrs   = arguments.length > 4 ? arguments[ 4 ] : null;
            var itype   = 'checkbox';
            var name    = hz.util.rand_id();

            //create the container element (list of selectable items)
            var ul = document.createElement( 'ul' );
            ul.className = 'check';

            //first bit indicates an exclusive selection widget
            if( flags & 1 ) {
                itype = 'radio';

                //the name applies to the entire group
            ...
            //first bit not set, inclusive selection widget
            else {
            ...
            var make_check_item = function( value, text, checked ) {
                ...
                //set name based on type of element
                var iname = itype == 'radio' ? name : ( name + '[]' );
            ...
            //generate change event handlers
            var checkit;
            ...
            //generate the list of selectable option widgets
            ...
            //set up a property to allow us to set the name of the element
            Object.defineProperty(
                ...
                'name',
                {
                    'enumerable' : true,
                    'get' : function() {
                        var cstr   = this.className.toString();
                        var fcname = this.firstChild.name;
                        if( cstr.match( /\binclusive\b/ ) != null ) {
                            var bpos = fcname.indexOf( '[' );
                            if( bpos != -1 ) {
                                return fcname.substr( 0, bpos );
                            }
                        }
                        return fcname;
                    },
                    'set' : function( value ) {
                        var cstr  = this.className.toString();
                        var nkids = this.childNodes.length;
                        if( cstr.match( /\bexclusive\b/ ) != null ) {
                            for( var i = 0; i < nkids; ++i ) {
                                this.childNodes[ i ].name = value;
                            }
                        }
                        else {
                            for( var i = 0; i < nkids; ++i ) {
        ...
        'efile' : function() {
            ...
            hz.util.oprx( parent, input, [ 'name' ] );
            return parent;
        },
        ...
        'gauge' : function() {
            ...
            var flags = arguments.length > 2 ? arguments[ 2 ] : 0;
            var gauge = new hz.doc.structure(
                'div,div:span',
                [ null, label, '' ]
            );
            gauge.bind_object( gauge, [ 'fill', 'content', 'status' ] );
            var element = gauge.get_element();
            element.className = 'gauge';
            element.hz = {
                'set_value' : function( value ) {
                    gauge.fill.style.width = ( value * 100.0 ) + '%';
                    if( flags & 1 ) {
                        gauge.status.firstChild.nodeValue
                            = ( value * 100.0 ).toFixed( 0 ) + '%';
                    }
                }
            };
            element.hz.set_value( value );
            return element;
        },

        /**
         *  Creates a link for use as a button.
         *
         *  @param value   The label text for the button
         *  @param handler An optional callback handler for the click event
         *  @return        The new input element
     ...
     *  @param handler  Optional handler to call for the form's submit event
